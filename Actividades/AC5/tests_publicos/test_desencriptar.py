import unittest
from main import (
    deserializar_tortuga,
    decodificar_largo,
    separar_msg_encriptado,
    decodificar_rango,
    desencriptar,
)
...
class TestDesencriptar(unittest.TestCase):
    ...
    def test_deserializar_tortuga_excepcion(self):
        ...
        self.assertRaises(AttributeError, deserializar_tortuga, no_deserializable)

        # Verificar que no levanta excepción
    ...
    def test_separar_msg_encriptado_par(self):
        ...
        with patch("main.decodificar_largo", side_effect=new_decodificar_largo):
            test = separar_msg_encriptado(
                bytearray(
                    b"\x00\x00\x02\x02\x01\x03\x00\x01\x00\x00\x00\x01\x00\x00\x04"
                )
            )
            res = [
                bytearray(b"\x02\x01"),
    ...
    def test_separar_msg_encriptado_impar(self):
        ...
        with patch("main.decodificar_largo", side_effect=decodificar_largo):
            test = separar_msg_encriptado(
                bytearray(
                    b"\x00\x00\x03\x44\x02\x01\x03\x00\x01\x02\x00\x00\x00\x01\x00\x00\x05"
                )
            )
            res = [
    ...
    def test_desencriptar(self):
        """
        Verificar correcto armado del mensaje encriptado asumiendo que "decodificar_largo", "separar" y "decodificar" están buenos
        """

        def separar_msg_encriptado(*args, **kwargs):
            return [
                bytearray(b"\x80\x02\x04"),
        ...
        with patch("main.decodificar_largo", side_effect=decodificar_largo):
            with patch(
                "main.separar_msg_encriptado", side_effect=separar_msg_encriptado
            ):
                with patch("main.decodificar_rango", side_effect=decodificar_rango):
                    test = desencriptar(
                        bytearray(
                            b"\x00\x00\x03\x04\x02\x80\x09\x00\x01\x02\x08\x00\x00\x01\x00\x00\x03"
                        )
                    )
